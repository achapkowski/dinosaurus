...
import json
from ...common._geom import Point, Polyline
from ...common._geom import Polygon
from ...common._geom import Envelope, MultiPoint
...
class GeometryService(BaseService):
    ...
    def areasAndLengths(self,
        ...
        if isinstance(polygons, list) and len(polygons) > 0:
            ...
            if isinstance(p, Polygon):
                ...
                params['polygons'] = [poly.as_dict for poly in polygons]
            elif isinstance(p, dict):
                params['polygons'] = [poly for poly in polygons]
            del p
    ...
    def __geometryListToGeomTemplate(self, geometries):
        ...
        if isinstance(geometries, list) and len(geometries) > 0:
            for g in geometries:
                ...
                template['geometries'].append(g.as_dict)
                del g
            return template
        return template
    #----------------------------------------------------------------------
    def __geometryToGeomTemplate(self, geometry):
        ...
        if isinstance(geometry, Polyline):
            ...
            raise AttributeError("Invalid geometry type")
        template['geometry'] = geometry.as_dict
        return template
    #----------------------------------------------------------------------
    def __geomToStringArray(self, geometries, returnType="str"):
        ...
        for g in geometries:
            if isinstance(g, Point):
                listGeoms.append(g.as_dict)
            elif isinstance(g, Polygon):
                listGeoms.append(g.as_dict)
            elif isinstance(g, Polyline):
                listGeoms.append({'paths' : g.as_dict['paths']})
        if returnType == "str":
    ...
    def convexHull(self,
        ...
        if isinstance(geometries, list) and len(geometries) > 0:
            ...
            if sr is not None:
                ...
                params['sr'] = g.spatialreference.as_dict
            if isinstance(g, Polygon):
                params['geometries'] = {"geometryType": "esriGeometryPolygon",
                                        "geometries" : self.__geomToStringArray(geometries, "list")}
            elif isinstance(g, Point):
    ...
    def cut(self,
        ...
        if isinstance(cutter, Polyline):
            params['cutter'] = cutter.as_dict
        elif isinstance(cutter, dict):
    ...
    def difference(self,
        ...
        if isinstance(geometry, Polyline):
            ...
            raise AttributeError("Invalid geometry type")
        geomTemplate['geometry'] = geometry.as_dict
    ...
    def findTransformation(self, inSR, outSR, extentOfInterest=None, numOfResults=1):
        ...
        if isinstance(extentOfInterest, Envelope):
            params['extentOfInterest'] = extentOfInterest.as_dict
    ...
    def reshape(self,
        ...
        if isinstance(reshaper, Polyline):
            params["reshaper"] = reshaper.as_dict
    ...
    def trimExtend(self,
        ...
        params = {
            ...
            "extendHow": extendHow,
            "trimExtendTo" : trimExtendTo.as_dict
